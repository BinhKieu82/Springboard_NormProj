...
FROM (SELECT f.name AS court,m.firstname AS firstname, m.surname AS surname	
    ...
    ON b.facid = f.facid AND f.name LIKE 'Tennis%'	
...
FROM Bookings AS b
INNER JOIN Facilities as f ON b.facid = f.facid AND b.starttime LIKE  '2012-09-14%'
...
SELECT concat(m.surname,' ',m.fistname) AS name, m.recommendedby AS recommended_ID, concat(r.firstname,' ',r.surname) AS recommendedby
FROM Members AS m 
...
SELECT sub.facid, COUNT(sub.memid) AS member_usage, f.name
...
SELECT sub.months, COUNT( sub.memid ) AS member_usage
